...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
  echo "[opencode-gradle] Starting persistent container..."
  docker-compose $ENV_FILE_ARG -f "$COMPOSE_FILE" run \
    --service-ports -d --name "$CONTAINER" gradle -c "sleep infinity" > /dev/null
  echo "[opencode-gradle] Container ready (ports: $GRADLE_PORT_1, $GRADLE_PORT_2)."
else
  BOUND=$(docker port "$CONTAINER" 2>/dev/null || true)
  for PORT in "$GRADLE_PORT_1" "$GRADLE_PORT_2"; do
    if ! echo "$BOUND" | grep -q ":${PORT}$"; then
      echo "[opencode-gradle] ERROR: Port $PORT is not bound on the running container."
      echo "[opencode-gradle] Port mappings are set at creation time. To apply new config:"
      echo "[opencode-gradle]   docker rm -f $CONTAINER"
      echo "[opencode-gradle] Then re-run this script."
      exit 1
    fi
  done
fi
